...
**Decision:** Strategy Settings are not one-time prompt fragments. A brand-keyed User Brief preserves:
...
Aliases such as `삼성전자` and `삼성 비스포크 정수기` are treated as one entity. External AI may not independently abbreviate or replace identity values.
...
Block unapproved visible `비즈넵/BIZNUP`, `삼쩜삼`, `더낸세금/혜움`, and `SSEM/쌤157`. Permit one only when legitimately included in the target or Registry/alias lock.
...
Keep Draft and unmerged until owner Preview approval.
