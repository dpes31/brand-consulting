...
export type FlowSlide = BaseSlide & {
  ...
  nodes: Array<{
    ...
    headline: string;
    detail?: string;
    tone?: 'neutral' | 'risk' | 'opportunity' | 'target';
  }>;
};
...
export type ProductionReportValidation = {
  ok: boolean;
  errors: string[];
};
...
export function validateProductionReport(
  ...
  if (report.version !== PRODUCTION_REPORT_VERSION) {
    errors.push(`Unsupported report version: ${String(report.version)}`);
  }
  ...
  if (report.mainSlides.length !== PRODUCTION_MAIN_PAGE_COUNT) {
    errors.push(`Main Deck must contain exactly ${PRODUCTION_MAIN_PAGE_COUNT} slides.`);
  }
  if (report.appendixSlides.length !== PRODUCTION_APPENDIX_PAGE_COUNT) {
  ...
  slides.forEach((slide, index) => {
    ...
    if (slide.zone !== expectedZone) {
      errors.push(`Slide ${slide.id} must use zone "${expectedZone}".`);
    }
