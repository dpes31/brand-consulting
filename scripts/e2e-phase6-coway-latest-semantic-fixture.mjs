...
function applyActualAppIdentityLock(documentRef) {
  for (let rank = 1; rank <= 3; rank += 1) {
    const name = documentRef.querySelector(`[data-report-field="comp-ranking.rank${rank}.name"]`)?.textContent?.trim();
    const summary = documentRef.querySelector(`[data-report-field="comp-ranking.rank${rank}.summaryName"]`);
    if (name && summary) summary.textContent = name;
  }
}

function applyLatestRawPresentationDrift(documentRef) {
  const breadcrumb = documentRef.querySelector('#creative-history-target .full-breadcrumb');
  if (breadcrumb) breadcrumb.textContent = fixture.criticalFixed.p29Breadcrumb;
  const arrow = documentRef.querySelector('#positioning .map-arrow-line');
  if (arrow) {
    Object.entries(fixture.criticalFixed.p18RawArrow).forEach(([key, value]) => arrow.setAttribute(key, value));
  }
}

function repairP12FromStep2SourceTruth(documentRef) {
  sourceTruthScores.forEach((scores, rankIndex) => {
    const rank = rankIndex + 1;
    Object.entries(scores).forEach(([role, value]) => {
      const element = documentRef.querySelector(`[data-report-field="comp-ranking.rank${rank}.${role}"]`);
