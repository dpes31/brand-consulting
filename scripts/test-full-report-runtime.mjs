...
assert.match(inputGuard, /looksLikeLegacyHtml/);
assert.match(inputGuard, /이전 HTML 생성 프롬프트로 만든 구형 HTML/);
assert.match(inputGuard, /ProductionReportV1 JSON/);

assert.match(bridge, /buildFullReportDataPrompt/);
assert.match(bridge, /assembleFullReportHtml/);
assert.match(bridge, /```json \.\.\. ``` 전체/);
